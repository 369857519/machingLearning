...
class Perceptron:
    ...
    def update(self, values, train, eta=.1):
        ...
        outputs. Updates according to the perceptron training
        ...
        count = 0
        for pointX in values:
            yi = self.activate(pointX)
            res = np.array([0.0, 0.0, 0.0])
            for j in range(len(pointX)):
                res[j] = self.weights[j] + eta * float(train[count] - yi) * float(pointX[j])
            self.weights = res
            count += 1
